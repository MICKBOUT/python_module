...
def process_event(event: dict) -> tuple:
    """
    print the actual event and return a tuple w/
    a True/False if the player is high level &
    the event index
    """
    is_high_level = event["data"]["level"] >= 10
    start = f"Event {event['id']}: Player {event['player']} \
(level {event['data']['level']})"
    if event["event_type"] == "login":
        print(start, "has login")
        return (is_high_level, 0)
    elif event["event_type"] == "logout":
        print(start, "has logout")
        return (is_high_level, 1)
    elif event["event_type"] == "death":
        print(start, "has died")
        return (is_high_level, 2)
    elif event["event_type"] == "kill":
        print(start, "killed monster")
        return (is_high_level, 3)
    elif event["event_type"] == "level_up":
        print(start, "leveled up")
        return (is_high_level, 4)
    elif event["event_type"] == "item_found":
        print(start, "found treasure")
        return (is_high_level, 5)
    return -1


def fibo():
    """
    calculate fibo to the end
    """
    yield 0
    yield 1
    n1, n0 = 1, 0
...
def prime_list():
    """
    return the next prim w/ yield
    """
    def is_prime(n: int) -> bool:
        """
        a helper function that calculate if a number is prim or not
        """
        if n <= 1:
            return False
        if n <= 3:
...
def event_stream():
    """
    selecte the events in the liste
    """
    for e in events:
        yield e
...
def list_getter():
    ...
    use event_stream to avoid the worst
    """
    return list(events)
...
def Game_data_stream_tester():
    ...
    for event in event_stream():
        (is_high_level, event_id) = process_event(event)
        if event_id == 4:
            count_event["level_up"] += 1
        elif event_id == 5:
            count_event["item_found"] += 1
        if is_high_level:
            high_level_count += 1

    print("\n=== Stream Analytics ===")
    print(f"Total event processed: {len(events)}")
    print(f"High-level players (10+): {high_level_count}")
    print(f"Treasure events: {count_event['item_found']}")
...
def generator_demonstration_tester():
    ...
    print(f"Fibonacci sequence (first 10): {[next(fi) for _ in range(10)]}")
    np = prime_list()
    print(f"Prime numbers (first 5): {[next(np) for _ in range(5)]}")


if __name__ == "__main__":
    Game_data_stream_tester()
    generator_demonstration_tester()
